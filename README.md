...
For content from a local clone, enter **Local repository**, or an **Attributes file location** inside a Git clone (the repository is then detected automatically). The converter locates the source module by its source ID, filename, or title and uses Asciidoctor to parse its containing guides. It captures inherited attributes at the actual include location, including nested assemblies, attribute restoration, conditional includes, and platform settings. Your pasted content is converted in memory; the clone is never modified.

One active inclusion is selected automatically. When a module belongs to several guides or appears under different contexts, **Guide containing this topic** offers the actual title and context of each inclusion. Choose the intended guide and convert again. **Repository source settings** lets you specify source and guide paths explicitly when discovery is ambiguous. The resolution details show the source, guide, include location, context, and resolved links.

This is a generic workflow: there are no hard-coded RHDH contexts, guide names, or directory requirements. Guide entry files may be named `master.adoc`, `index.adoc`, or something else. RHDH metadata conventions are supported alongside explicit type selection for other repositories. Discovery proposes candidates; native Asciidoctor parsing determines which inclusions are active. A missing value is never guessed.

**Attribute overrides** accepts `name=value`, one per line, and takes precedence over file and guide definitions. Use these to choose a build variant or provide values normally supplied by an external build configuration. The guide supplies values such as `context` automatically; a shared attributes file alone may omit them. For standalone conversion, definitions can also be supplied explicitly:
...
Without a repository or attributes file, pasted and uploaded files are self-contained. An attributes file outside a Git clone is read directly, with includes confined to its folder. Missing attributes are listed once in the UI; unsupported content produces diagnostics instead of silently disappearing. Existing explicit IDs are preserved. Files without an ID receive a stable ID derived from their logical source path.

For guide conversions, a uniquely identified topic-root anchor becomes a relative DITA XML link, such as `../shared/install.xml#install_guide-context`. Only warnings for proven targets are removed. Unknown or ambiguous targets, assembly/map targets, and nested-section anchors retain review warnings. Referenced topics are not generated by a single-topic download: convert them using the same guide and preserve repository-relative output folders. The converter does not run repository scripts or infer externally supplied build flags.
